...
namespace CacheMeIfYouCan.Tests
{
    ...
    public class TwoTierCacheTests2
    {
        ...
        public void GetMany_ChecksLocalCacheThenDistributedCache()
        {
            ...
            twoTierCache.GetMany(1, Enumerable.Range(0, 100).ToArray()).Result.Should().BeEmpty();
            localCache.GetManyExecutionCount.Should().Be(1);
            distributedCache.GetManyExecutionCount.Should().Be(1);
            
            twoTierCache.SetMany(1, Enumerable.Range(0, 50).Select(i => new KeyValuePair<int, int>(i, i)).ToArray(), TimeSpan.FromSeconds(1));
            localCache.SetMany1ExecutionCount.Should().Be(1);
            distributedCache.SetManyExecutionCount.Should().Be(1);

            var results1 = twoTierCache.GetMany(1, Enumerable.Range(0, 100).ToArray()).Result;
            ...
            var results2 = twoTierCache.GetMany(1, Enumerable.Range(0, 50).ToArray()).Result;
        ...
        public void GetMany_SetsValueInLocalCacheIfFoundInDistributedCache()
        {
            ...
            distributedCache
                .SetMany(1, Enumerable.Range(0, 50).Select(i => new KeyValuePair<int, int>(i, i)).ToArray(), TimeSpan.FromSeconds(1))
                .Wait();
            
            distributedCache.SetManyExecutionCount.Should().Be(1);

            var values1 = twoTierCache.GetMany(1, Enumerable.Range(0, 100).ToArray()).Result;
            ...
            var values2 = twoTierCache.GetMany(1, Enumerable.Range(0, 50).ToArray()).Result;
